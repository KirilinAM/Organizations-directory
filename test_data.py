...
from app.database.models import Organization, Phone, Building, Activity, Organization_Activity_Rel
...
async def create_test_data():
    async with asyncSessionMaker() as session:
        ...
        bakery_activity = Activity(name="Хлебобулочные изделия", parent_id=food_activity.id)
        ...
        computers_activity = Activity(name="Компьютеры", parent_id=electronics_activity.id)
        phones_activity = Activity(name="Телефоны", parent_id=electronics_activity.id)
        
        session.add_all([
            meat_activity, dairy_activity, bakery_activity,
            trucks_activity, cars_parts_activity,
            computers_activity, phones_activity
        ])
        await session.flush()
        
        # Уровень 3 для Легковых автомобилей
        car_parts_activity = Activity(name="Запчасти", parent_id=cars_parts_activity.id)
        car_accessories_activity = Activity(name="Аксессуары", parent_id=cars_parts_activity.id)
        
        # Уровень 3 для Компьютеров
        laptops_activity = Activity(name="Ноутбуки", parent_id=computers_activity.id)
        components_activity = Activity(name="Комплектующие", parent_id=computers_activity.id)
        ...
        session.add_all([
            car_parts_activity, car_accessories_activity,
            laptops_activity, components_activity,
            beef_activity, pork_activity, poultry_activity
        ])
        await session.flush()
        ...
        organization_activities = [
            ...
            Organization_Activity_Rel(organization_id=organizations[0].id, activity_id=meat_activity.id),
            Organization_Activity_Rel(organization_id=organizations[0].id, activity_id=dairy_activity.id),
            Organization_Activity_Rel(organization_id=organizations[0].id, activity_id=car_parts_activity.id),
            
            # АО "МолПродукт" - специализируется на молочной продукции
            Organization_Activity_Rel(organization_id=organizations[1].id, activity_id=dairy_activity.id),
            
            # ИП Иванов - мясная продукция разных видов
            Organization_Activity_Rel(organization_id=organizations[2].id, activity_id=beef_activity.id),
            Organization_Activity_Rel(organization_id=organizations[2].id, activity_id=pork_activity.id),
            
            # ЗАО "АвтоДеталь" - автомобильные запчасти и аксессуары
            Organization_Activity_Rel(organization_id=organizations[3].id, activity_id=car_parts_activity.id),
            Organization_Activity_Rel(organization_id=organizations[3].id, activity_id=car_accessories_activity.id),
            
            # ООО "ТехноМир" - электроника и компьютеры
            Organization_Activity_Rel(organization_id=organizations[4].id, activity_id=laptops_activity.id),
            Organization_Activity_Rel(organization_id=organizations[4].id, activity_id=components_activity.id),
            Organization_Activity_Rel(organization_id=organizations[4].id, activity_id=phones_activity.id),
        ]
        ...
        print(f"Создано: {len([a for a in session.identity_map.values() if isinstance(a, Activity)])} видов деятельности")
